...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  {
  case adventurer:
    adventurer_function(state); //CHANGED
    break;

  case council_room:
    council_room_function(state, handPos); //CHANGED
    break;
  ...
  case remodel:
    remodel_function(state, choice1, choice2, currentPlayer, handPos);
    break;

  case smithy:
    smithy_function(state, handPos); //CHANGED
    break;
  ...
  case great_hall:
    great_hall_function(state, handPos); // CHANGED
    break;
...
int adventurer_function(struct gameState *state)
{
  ...
  while (drawntreasure < 1) { // BUG s/b 2
...
int remodel_function(struct gameState *state, int choice1, int choice2, int currentPlayer, int handPos)
{
  int i = 0;
  int j = state->hand[currentPlayer][choice1];  //store card we will trash

  if ( (getCost(state->hand[currentPlayer][choice1]) + 2) < getCost(choice2) ) // BUG s/b >
  {
    return -1;
  }
...
int council_room_function(struct gameState *state, int handPos)
{
  ...
  //+1 Buy
  // state->numBuys++; //BUG should not be commented
