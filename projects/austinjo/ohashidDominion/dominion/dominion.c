...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      	useAdventurer(currentPlayer, state, handPos);
	return 0;
    ...
    case great_hall:
      	useGreathall(currentPlayer, state, handPos);
	return 0;
...
int useSmithy(int currentPlayer, struct gameState *state, int handPos){
	int i;
	//bug introduced to loop four times instead of 3
	for(i=0; i<4; i++){
		drawCard(currentPlayer, state);
	}
	discardCard(handPos, currentPlayer, state, 0);
...
int useGreathall(int currentPlayer, struct gameState *state, int handPos){
	//bug introduced to allow drawing of two cards instead of one
	drawCard(currentPlayer, state);	
	drawCard(currentPlayer, state);
	//+1 Action
	state->numActions++;
...
int useAdventurer(int currentPlayer, struct gameState *state, int handPos){
	//bug introduced to intialize drawntreasure to 1 to only allow drawing of up to 1 treasure
	int drawntreasure = 1;
	int cardDrawn;
...
int useVillage(int currentPlayer, struct gameState *state, int handPos){
	drawCard(currentPlayer, state);
	//bug introduced to increase number of actions to 3 from 2
